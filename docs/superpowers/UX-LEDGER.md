...
| 20 | 07-11 | Commit `Screenshots for GUI etc/` — the CD-UI digest cites cd-01..cd-23 / fabled-01..03 from an **untracked** folder | **Decided 2026-07-12: `.gitignore` it** (DECISIONS "4c Work Stream B plan LOCKED") — make the intentional untrack explicit; 46 MB / 27 PNGs stay out of history. Done in 4c plan Task 0 |
...
| 28 | 07-12 | **Git footer strip** — branch · session ±diff · time · cost · ~~Create PR~~ | Adopted from digest §2b (cd-14). Quiet transcript-footer at the bottom of the conversation view. 🎨 design-decided. **v1 ships read-only (branch·±diff·cost); Create PR omitted, Ben 2026-07-12** (DECISIONS "4c Work Stream B plan LOCKED") — returns later as its own outward-facing feature |
| 29 | 07-12 | **Mode-aware appearance** — follows the system light/dark setting, no toggle | ✅ **CLOSED — Ben verified in the B0 build 2026-07-12: "yes the app was mode responsive."** Light Linen + dark Teal Midnight both render from the locked palette and flip live; Ben kept both values as-is (dark reads near-black on a bare screen by design — teal identity arrives with the B1/B2 accents). |
