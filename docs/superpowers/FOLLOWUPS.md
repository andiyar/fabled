...
- **Notification gate-bursts aren't coalesced (T11 review).** Each `gateArrived` posts its own banner; a rapid multi-gate flow would stack them. If multi-gate flows appear in practice, reuse a stable notification identifier keyed on sessionID so a newer gate replaces the older banner. → 4c if felt.
